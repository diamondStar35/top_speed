    ...
    internal sealed partial class MenuRegistry
    {
        ...
        private List<MenuItem> BuildGeneralSettingsItems()
        {
            var items = new List<MenuItem>
            {
                ...
                new CheckBox(LocalizationService.Mark("Enable digit navigation globally"),
                    () => _settings.MenuDigitActivation,
                    value => _settingsActions.UpdateSetting(() => _settings.MenuDigitActivation = value),
                    onChanged: value => _menu.SetMenuDigitActivation(value),
                    hintProvider: HintToggleProvider(LocalizationService.Mark("When unchecked, pressing a number key activates the item at that position in certain areas, such as the pit stop menu. When checked, it works everywhere."))),
