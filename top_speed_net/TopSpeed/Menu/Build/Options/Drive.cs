    ...
    internal sealed partial class MenuRegistry
    {
        private MenuScreen BuildOptionsDriveSettingsMenu()
        {
            var items = new List<MenuItem>
            {
                ...
                new CheckBox(LocalizationService.Mark("Brief status reports"),
                    () => _settings.BriefStatusReports,
                    value => _settingsActions.UpdateSetting(() => _settings.BriefStatusReports = value),
                    hintProvider: HintToggleProvider(LocalizationService.Mark("When checked, spoken status reports are shortened for experienced players: labels such as \"lap percentage\" and \"gear\" are dropped, and the number row gives a quick position rundown."))),
                new CheckBox(LocalizationService.Mark("Report lap and turn instead of race percentage"),
                    () => _settings.ReportLapAndTurn,
                    value => _settingsActions.UpdateSetting(() => _settings.ReportLapAndTurn = value),
                    hintProvider: HintToggleProvider(LocalizationService.Mark("When checked, the number row and player information report a car's lap and the turn it is in or approaching, spoken like a spotter, instead of its race percentage. This applies to your own car and to any other driver you check with the number row. Turns are numbered from the track's curves, so the count may differ from a real circuit's official turn numbers."))),
